...
- Canvas 2D：为可读图生成 64×64 touch texture 和头像预处理；为无 CORS 头像执行 tainted draw、点阵合成与局部网格位移。
...
- `_qa/capture.mjs`：默认头像、调试头像、基线、平台桥 + 无 CORS 头像，以及双尺寸触摸前 / 位移峰值 / 回弹 / 完成态。

## 3. 核心模块

身份解析顺序为 `?avatar_url=` → canonical Aigram 资料接口的 `data.head_url`
→ AlterU 默认头像。资料成功与像素可读性分开判断。平台 R2 头像不设置
`crossOrigin=anonymous`。浏览器允许把跨域图片绘入 Canvas，只是会阻止
`getImageData()`、导出和读回；本作不需要读回。头像先按 center-cover 绘入
720×720 source canvas，再以 `destination-in` 合成独立生成的 180×180
随机圆粒 mask。显示 canvas 把该 source 分成 36×36 网格重绘，因此可以只靠
`drawImage()` 改变局部位置。该路径避免 32×32 碎片造成的大马赛克、照片重复
和分区接缝，也不再把真实头像降成静态点阵图。仅当真实头像 URL 无法加载时才使用
...
收束到 1.5。平台动态场在每次 Pointer 输入时，以 22% 画布宽度为影响半径，
向相邻网格注入径向速度和较小切向速度，同时直接写入一次同帧位移；RAF 用
弹簧力与阻尼完成峰值和回弹，位移/速度低于 0.08 后停止 Canvas 重绘。覆盖进度
让整张点阵肖像从 1.018 倍、84% 不透明度收束到 1 倍、100%；重复触摸仍产生
局部位移和电流环。

## 4. 扩展点

- 改粒子算法：编辑 `src/shaders/particle.vert|frag`，build 会重编 vertex。
- 改头像裁切/尺寸：编辑 `prepareImage()`、`createParticleMaskCanvas()` 与 `.portrait-mask`。
- 改平台头像触摸动力：编辑 `addMaskedImpulse()` 与 `updateMaskedMotion()` 的半径、推力、旋流、弹簧、阻尼和位移上限。
...
`index.html` 的内联点阵桥先于模块和头像请求绘制。WebGL 路径在 `particleMesh` 已存在且 `renderer.render()` 完成后交接；平台路径在真实头像完成 tainted draw、点阵合成、动态画布首帧并进入一个动画帧后交接。两者都设置 `body[data-visual-ready=true]`，再移除启动桥。
