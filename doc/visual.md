...
平台头像缺失或图片本身加载失败时才明确回退。平台 R2 头像能显示但没有 CORS 像素读取权限时，必须保留真实用户头像并切换到 24×24 DOM 图像碎片场；不得静默改用缺省图。`?baseline=1` 是唯一使用原示例图的默认入口。两种渲染路径都按 center-cover 保持正向、比例和 RGB 色彩。
...
- 动效：可读图使用原 shader 噪声与触摸波；平台图使用碎片散射、区域收束和 520ms 电流环；HUD 反馈 160–260ms。
...
390×844 与 320×568 必须覆盖：基线示例、调试头像覆盖、默认头像回退、平台桥 + 无 CORS 玩家头像、9/12 完成态；断言身份来源仍为 `player`，并确认头像不倒置、不拉伸、不溢出、触摸反馈和重置可达。
